...
const CATALOG = [
  { key: "agent_base_url", group: "agent", kind: "text", label: "Agent server URL", help: "Where questions are sent.", optional: false },
...
test("an empty box and a box of spaces are both 'no opinion'", () => {
  ...
  assert.equal(saying(spec("agent_base_url"), { mode: SYSTEM, raw: "" }), null);
  assert.equal(saying(spec("agent_base_url"), { mode: SET, raw: "   " }), null);
  assert.equal(saying(spec("agent_base_url"), undefined), null);
  assert.equal(saying(spec("agent_base_url"), { mode: SET, raw: " http://a " }), "set:http://a");
});
...
test("dirty keys are the edits the server has not been told about", () => {
  const saved = fromStored(CATALOG, { agent_base_url: "http://a" });
  ...
  const cleared = { ...saved, agent_base_url: { mode: SYSTEM, raw: "" } };
  assert.deepEqual(dirtyKeys(CATALOG, cleared, saved), ["agent_base_url"]);

  // Retyping the same value is not.
  const retyped = { ...saved, agent_base_url: { mode: SET, raw: "http://a" } };
...
test("search reads the label, the help, the group and the key", () => {
  assert.equal(matchesQuery(spec("agent_base_url"), "Agent", "url"), true);
  assert.equal(matchesQuery(spec("agent_base_url"), "Agent", "questions"), true);
  assert.equal(matchesQuery(spec("agent_base_url"), "Agent", "agent_base"), true);
  ...
  assert.equal(matchesQuery(spec("agent_base_url"), "Agent", "agent timeout"), false);
  assert.equal(matchesQuery(spec("agent_base_url"), "Agent", "   "), true);
});
...
test("a group's override count reports the group, not the search", () => {
  const form = fromStored(CATALOG, { agent_base_url: "http://a", agent_timeout_s: 30 });
