# ⚠️ 已被取代 — 請讀 [`spec_v2.md`](spec_v2.md)

> **這份文件是歷史存檔，不再維護。**
>
> **要理解這個專案，請讀 [`docs/spec_v2.md`](spec_v2.md)** —— 那是唯一權威的技術文件，
> 自包含、結構乾淨，並且**包含 Stage 4（Playground），本文件完全沒有提到它**。
> 兩者有衝突時一律以 `spec_v2.md` 為準。
>
> **本文件為什麼還留著**：
> 1. 它是**討論過程的紀錄**，保留了一些後來被推翻的假設。想知道「為什麼沒選另一條路」時有用。
> 2. 程式碼註解大量引用它的章節編號（`§6.9`、`§9.2`…）。`spec_v2.md` 在對應處標注了舊編號，
>    但直接查閱原文有時更快。
>
> **本文件的已知陷阱**（`spec_v2.md` 已全部修正）：
> - §1.1／§6.2 寫的 **A2A protocol 已經不是現況**（agent server 改成單一 `POST /execute`）。
> - **只有四個 seam**；實際上有五個（第五個是 Playground 的 skill 目錄）。
> - §1–§8 是設計脈絡而非程式碼描述；§9 才是 Stage 1 的實作現況；**Stage 4 完全不在本文件內**。

---

# Agent Evaluation + Trace 錯誤定位 + Skill Evolving 系統 — Spec 討論輸入文件（歷史存檔）

> 本文件用途（原始）：作為與 Claude chat 後續討論的輸入，目標是把下述系統擬定成一份詳細的
> system spec。以下內容包含 (1) 背景情境、
