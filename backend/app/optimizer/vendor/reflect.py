...
from app.optimizer.vendor.skill_aware import (
...
def fmt_minibatch_trajectories(
    ...
    ``"task_type"``, ``"fail_reason"``, etc.  Takes the conversation from the
    item itself, or reads ``conversation.json`` for it, and formats them
    together with trajectory headers.
    ...
    ----------
    items : list[dict]
        Rollout result dicts belonging to one minibatch. An item may carry its
        conversation inline under ``"conversation"``.
    prediction_dir : str | None
        Path to ``predictions/`` directory containing per-task
        ``<task_id>/conversation.json`` files, for items that do not.
    ...
    for idx, item in enumerate(items, 1):
        tid = str(item["id"])
        # An in-memory conversation is the only source here. Upstream's
        # trajectories are files a local rollout wrote; ours are Langfuse
        # traces, and they arrive already truncated to the reflect budget —
        # doing that *before* formatting is what makes the analyst prompt
        # bounded rather than as long as the agent happened to be chatty
        # (see `app/optimizer/reflection.py`). The file path is kept as the
        # fallback so this function still behaves as upstream's when given
        # upstream's inputs.
        ...
        if not target_prompt and prediction_dir:
        ...
        if not user_prompt and prediction_dir:
        ...
        if os.environ.get("REFLACT_CODEX_TRACE_TO_OPTIMIZER", "0") == "1":
            ...
            if not codex_trace_summary and prediction_dir:
        ...
        if not preview and prediction_dir:
